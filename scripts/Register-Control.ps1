...
function Register-MessageBoxDll {
    ...
    if ($DoCheck) {
        if (Test-MessageBoxControlLoaded) {
            Write-Verbose "[Register-MessageBoxDll] already loaded"
            return;
        }
    }

    $DllPath = Get-MessageBoxDllPath $Target
    if (($DllPath) -and (Test-Path $DllPath)) {
        Write-Verbose "[Register-MessageBoxDll] Add-Type -Path `"$DllPath`""
...
function Unregister-MessageBoxDll {
    [CmdletBinding(SupportsShouldProcess)]
    param(
    ...
    Write-Verbose "Get-RegListItemList RegisteredMessageBoxControlProcessId"
        ...
        foreach ($strProcessId in $ProcessesFlagged) {
            ...
            if ("$pid" -eq "$strProcessId") {
                ...
                if ($ProcessPtr) {
                    ...
                    $StrLog = "MessageBoxDll {0} ({1})" -f $pname, $Status
...
function Test-MessageBoxControlLoaded {
    [CmdletBinding(SupportsShouldProcess)]
    param()

    try {
        $Obj = [MessageBox.WarningDialog] -as [type]
        return $True
    } catch {
        return $False
    }
